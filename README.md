# Racing Sim
This is a repository for tracking progresses and history about all things `Racing Sim` that are related to my own rigs. This `README` Section will log all the progress below, showing the most recent changes first. Each project folder may be referred in this document for more details, and items.

## Current Setup
### Controllers
- MOZA racing R5
- MOZA racing ES wheel
- MOZA racing Sequential Shifter
- MOZA racing CRP2 Throttle
- MOZA racing mBoost Active Pedal(Brake)
- Cheap Chineses Hall sensor Handbrake(not mounted yet)

### Display
- TCL 55" QM6K x3 @ 1440p, 120hz
- LG 32GK650F @ 1440p, 60hz

### Computer Hardware
- AMD Ryzen 7 5800X3D
- Asus TUF RTX 5090
- 128GB memory DDR4 3200Mhz
- Logitech K400 Wireless keyboard+touchpad
- 4 Channels of 25W bass shakers driven by two small Class D amps (only three channels are used for now)

### Frame
- (Custom built wooden frame)[https://github.com/tmitb/sim-rig-racing/tree/main/wooden-frame#readme]

### Noteworthy accessories
- Ventilated seat pad (Aliexpress link)[https://www.aliexpress.com/item/1005007196227825.html?spm=a2g0o.order_list.order_list_main.5.4b971802pqZyDx]
  - I absolutely love it.
  - If it gets too hot and you sweat seating in too long for a session, this is a great accessory to get under $50.
  - You need to get 12v DC adapter from somewhere. I had one laying around in my DC adaptor pile, but you may need to get one. It requires 2A at 12v.
  - It is a bit noisy, so if you want to speak with an open mic, probably not a good option.


# License
Unless specified otherwise, All 3D models are CC BY-SA 4.0, and software portion is MIT License.

# History Log
## Setup updated (2026-1-13)
I have moved on to triple display setup. With it, I had finished couple phases of my custom built wooden frame project. For more information, follow the link above in the frame section.

## Foot rest/Riser added (2025-11-30)

## Repo is born (2025-11-26)
The repo is born this day with something inside for the first time. I am in process of collecting some data for the current sim setup, which is quite meassy to say the least.


